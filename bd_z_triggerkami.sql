...
CREATE TRIGGER ok_lot BEFORE INSERT OR UPDATE ON loty FOR EACH ROW EXECUTE PROCEDURE sprawdz_lot();
...
CREATE OR REPLACE FUNCTION spr_akt_statusu() RETURNS TRIGGER AS $$
DECLARE
n_st_old VARCHAR(20);
n_st VARCHAR(20);
    BEGIN
        SELECT s.nazwa INTO n_st FROM status s WHERE s.id_statusu=NEW.id_statusu;
...
CREATE TRIGGER aktual_status BEFORE UPDATE ON loty FOR EACH ROW EXECUTE PROCEDURE spr_akt_statusu();
